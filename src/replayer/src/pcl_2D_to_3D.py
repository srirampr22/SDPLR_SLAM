...
class ImageProcessor:
    def __init__(self):
        ...
        self.save_points_flag = False

        # Start the thread to listen for keyboard events
            ...
            if image_msg.header.stamp != depth_msg.header.stamp:
                ...
                if self.intrinsics is not None:
                    ...
                    if len(self.bboxes) == 0:
                        ...
                        print("POINTSSSSSSSSSSSSSSSSSSSSSSSSSSSSS:",points.shape)
    ...
    def save_transformed_points(self, folder_path):
        if self.transformed_points is not None:
            # Ensure that the folder_path ends with a forward slash '/'
            if not folder_path.endswith('/'):
                folder_path += '/'

            # Concatenate folder_path with the desired filename
            file_path = os.path.join(folder_path, '60_inch_mean_cov.csv')

            # Save the transformed points to the specified file path
            np.savetxt(file_path, self.transformed_points, delimiter=',')
            print(f"Transformed points saved to '{file_path}'")
        else:
            print("No transformed points available.")
    ...
    def pointcloud_callback(self, pcl_msg):
        ...
        if self.foreground_points is not None:
            ...
            transformed_points =  ROI_mean_points

            self.transformed_points = transformed_points

            if self.save_points_flag:
                self.save_transformed_points(folder_path)
                self.save_points_flag = False  # Reset the flag
            ...
            # # relabel because DBSCAN labels are random and not maintained through time
        ...
        #     # If foreground_points is None and no subjects have been seen before, do nothing
        #     if self.latest_subject_positions_map is None:
        #         return None

        #      # Publish the last known position of each subject from the dictionary
        #     if self.latest_subject_positions_map:
        #         # Get the last known positions from the dictionary
        #         last_known_positions = list(self.latest_subject_positions_map.values())

        #         # Stack the positions to create a single point cloud message
        #         last_known_positions = np.vstack(last_known_positions)

        #         # Convert the last_known_positions to a point cloud message
        #         last_known_pcl_array = self.XYZRGB_to_pcl_mat(last_known_positions)
        #         last_known_pc_array = merge_rgb_fields(last_known_pcl_array)
        #         last_known_pc_msg = ros_numpy.msgify(
        #             PointCloud2, last_known_pc_array, stamp=pcl_msg.header.stamp, frame_id='map')  # Set the frame_id to 'map'
        #         self.map_frame_pub.publish(last_known_pc_msg)
...
if __name__ == '__main__':
    ...
    folder_path = '/home/sriram/DPLR/dplr_ws/src/replayer/src/ref_mean_cov' 
